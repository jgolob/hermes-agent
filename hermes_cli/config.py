...
def ensure_hermes_home():
    ...
    # Official container bootstrap enforces once before unprivileged setup and
    # once at its final boundary. Avoid walking the volume again in every
    # supervised Python process; native processes have no external bootstrap,
    # so this is their once-per-start enforcement point.
    if is_shared_hermes_home() and not _is_container():
        from hermes_constants import enforce_shared_hermes_home

        enforcement_errors = enforce_shared_hermes_home(
            get_default_hermes_root()
        )
        if enforcement_errors:
            logger.warning(
                "HERMES_SHARED_HOME could not repair %d operation(s):\n%s",
                len(enforcement_errors),
                "\n".join(f"  {error}" for error in enforcement_errors),
            )
